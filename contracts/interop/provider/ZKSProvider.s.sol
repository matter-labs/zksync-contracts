...
import {IL1AssetRouter} from "../../l1-contracts/bridge/asset-router/IL1AssetRouter.sol";
import {IL1Nullifier, FinalizeL1DepositParams} from "../../l1-contracts/bridge/interfaces/IL1Nullifier.sol";
import {IGetters} from "../../l1-contracts/state-transition/chain-interfaces/IGetters.sol";
...
contract ZKSProvider is Script {
    ...
    function getL2ToL1MessageData(
        ...
        for (uint256 i = 0; i < receipt.logs.length; i++) {
            if (receipt.logs[i].addr == L1_MESSENGER_ADDRESS &&
