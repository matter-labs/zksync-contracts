...
import {
  ...
  ZeroAddress
} from "../l1-contracts/common/L1ContractErrors.sol";
import { InteropCallStarter } from "../l1-contracts/common/Messaging.sol";
import {
  L2_ASSET_ROUTER,
...
library InteropLibrary {
  ...
  function buildSecondBridgeCalldata(
    ...
    bytes memory inner =
      DataEncoding.encodeBridgeBurnData(amount, receiver, maybeTokenAddress);
    return
      DataEncoding.encodeAssetRouterBridgehubDepositData(l2TokenAssetId, inner);
  }
  ...
  function buildSecondBridgeCall(
    ...
    callAttributes[0] =
      abi.encodeCall(IERC7786Attributes.indirectCall, (uint256(0)));
  ...
  function buildCall(
    ...
    bytes[] memory bundleAttributes =
      buildBundleAttributes(executionAddress, unbundlerAddress);
  ...
  function buildBundleCall(address target, bytes memory data)
    internal
    pure
  ...
  function buildSendDestinationChainBaseTokenCall(
    ...
    bytes32 destinationBaseTokenAssetId =
      L2_BRIDGEHUB.baseTokenAssetId(destination);
    ...
    callAttributes[0] = abi.encodeCall(
      IERC7786Attributes.interopCallValue, (indirectCall ? 0 : amount)
    );
    if (indirectCall) {
      callAttributes[1] =
    ...
    return InteropCallStarter({
      ...
      data: indirectCall
        ? buildSecondBridgeCalldata(
          thisChainBaseTokenAssetId, amount, recipient, address(0)
        )
        : empty,
      callAttributes: callAttributes
    });
  ...
  function buildBundleAttributes(address unbundlerAddress)
    internal
    pure
    returns (bytes[] memory attrs)
  {
    attrs = new bytes[](1);
  ...
  /// @notice Build a call-level 7786 attributes array.
  function buildCallAttributes(bool indirectCall)
    internal
    pure
  ...
  function sendToken(
    ...
    return L2_INTEROP_CENTER.sendBundle(
      InteroperableAddress.formatEvmV1(destinationChainId), calls, bundleAttrs
    );
  }
  ...
  function sendDirectCallBundle(
    ...
    bytes[] memory bundleAttrs =
      buildBundleAttributes(executionAddress, unbundlerAddress);

    return L2_INTEROP_CENTER.sendBundle(
      InteroperableAddress.formatEvmV1(destination), calls, bundleAttrs
    );
  }
  ...
  function sendNative(
    ...
    InteropCallStarter[] memory calls = new InteropCallStarter[](1);
    calls[0] = buildSendDestinationChainBaseTokenCall(
      destinationChainId, recipient, amount
    );
    bytes[] memory bundleAttributes = buildBundleAttributes(unbundlerAddress);

    return L2_INTEROP_CENTER.sendBundle{ value: amount }(
  ...
  function _concatBytesArrays(bytes[] memory a, bytes[] memory b)
    internal
    pure
