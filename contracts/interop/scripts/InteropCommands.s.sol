...
contract InteropScripts is Script, ZKSProvider {
  ...
  function sendMessage(string memory l2RpcUrl, bytes memory message)
    public
    returns (bytes32 txHash)
  {
    ...
    bytes memory callData =
      abi.encodeWithSelector(IL2ToL1Messenger.sendToL1.selector, message);

    txHash = _castSendRaw(
      l2RpcUrl, privateKey, L2_TO_L1_MESSENGER_SYSTEM_CONTRACT_ADDR, callData
    );
    console.log(
      "L2->L2 message sent (verify on destination with proveL2MessageInclusionShared)"
    );
  ...
  function sendMessageString(string memory l2RpcUrl, string memory message)
    public
  ...
  function withdrawToL1(
    ...
    bytes memory callData =
      abi.encodeWithSelector(IBaseToken.withdraw.selector, l1Receiver);

    txHash = _castSendRawWithValue(
      l2RpcUrl, privateKey, L2_BASE_TOKEN_SYSTEM_CONTRACT_ADDR, callData, amount
    );
  ...
  function withdrawToL1WithMessage(
    ...
    bytes memory callData = abi.encodeWithSelector(
      IBaseToken.withdrawWithMessage.selector, l1Receiver, additionalData
    );
  ...
  function sendBundle(
    ...
    bytes[] memory bundleAttributes =
      InteropLibrary.buildBundleAttributes(executionAddress, unbundlerAddress);
  ...
  function _buildDirectCall(address target, bytes memory data)
    internal
    pure
    returns (InteropCallStarter memory)
  {
  ...
  function sendToken(
    ...
    bundleHash = txHash;
    console.log(
      "Use this tx hash with executeInteropBundle to complete the transfer on destination chain"
    );
  }
  ...
  function _buildSendBundleData(
    ...
    calls[0] = InteropLibrary.buildSecondBridgeCall(
      secondBridgeCalldata, L2_ASSET_ROUTER_ADDR
    );
  ...
  function _getBundleHashFromTx(string memory l2RpcUrl, bytes32 txHash)
    internal
    returns (bytes32)
  {
  ...
  function _castSendRaw(
    ...
    txHash =
      abi.decode(vm.parseJson(string(result), ".transactionHash"), (bytes32));
  }
  ...
  function _buildSendDirectCallData(
    ...
    bytes[] memory mergedAttributes =
      new bytes[](call.callAttributes.length + bundleAttributes.length);
  ...
  function executeInteropBundle(
    ...
    TransactionReceipt memory receipt =
      getTransactionReceipt(sourceL2RpcUrl, sendBundleTxHash);
    ...
    console.log(
      "Step 2: Fetching L2ToL1 log proof (waiting for batch to be sealed)..."
    );
    L2ToL1LogProof memory logProof =
      getL2ToL1LogProof(sourceL2RpcUrl, sendBundleTxHash, 0);
    ...
    bytes memory executeBundleCalldata = abi.encodeWithSelector(
      IInteropHandler.executeBundle.selector, encodedBundle, proof
    );
  ...
  function getBundleHash(string memory l2RpcUrl, bytes32 txHash)
    public
  ...
  function getBundleStatus(string memory l2RpcUrl, bytes32 bundleHash)
    public
  ...
  function getTokenInfo(
    ...
    assetId =
      IL2NativeTokenVault(L2_NATIVE_TOKEN_VAULT_ADDR).assetId(tokenAddress);
    ...
    wrappedTokenAddress =
      IL2NativeTokenVault(L2_NATIVE_TOKEN_VAULT_ADDR).tokenAddress(assetId);
  ...
  function getTokenBalance(
    ...
    (bytes32 assetId, address wrappedTokenAddress) =
      getTokenInfo(sourceL2RpcUrl, destL2RpcUrl, tokenAddress);
  ...
  function sendSingleMessage(
    ...
    sendId = InteropLibrary.sendDirectCall(
      destinationChainId, target, payload, executionAddress, unbundlerAddress
    );
  ...
  function verifyMessageDelivery(
    ...
    TransactionReceipt memory receipt =
      getTransactionReceipt(sourceRpcUrl, txHash);
    ...
    waitForInteropRoot(
      destRpcUrl, sourceChainId, logProof.batchNumber, bytes32(0), 120
    );
  ...
  function encodeERC7930(uint256 chainId, address addr)
    public
    pure
  ...
  /// @param addr The address
  /// @return encoded The ERC-7930 encoded bytes
  function encodeERC7930Address(address addr)
    public
    pure
    returns (bytes memory encoded)
  {
  ...
  /// @param chainId The chain ID
  /// @return encoded The ERC-7930 encoded bytes
  function encodeERC7930ChainId(uint256 chainId)
    public
    pure
    returns (bytes memory encoded)
  {
  ...
  function computeAssetId(string memory l2RpcUrl, address tokenAddress)
    public
  ...
  function _getInteropBundle(TransactionReceipt memory receipt)
    internal
    view
    returns (InteropBundle memory bundle)
  {
    ...
    for (uint256 i = 0; i < logsLen; ++i) {
      ...
      (,, bundle) =
        abi.decode(receipt.logs[i].data, (bytes32, bytes32, InteropBundle));
      return bundle;
    }
