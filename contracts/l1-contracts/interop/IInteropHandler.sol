...
interface IInteropHandler {
  ...
  event CallProcessed(
    bytes32 indexed bundleHash, uint256 indexed callIndex, CallStatus status
  );
  ...
  function executeBundle(
    ...
    InteropMessageProof memory _proof
  ) external;

  /// @notice Verifies receipt of a bundle without executing calls.
  /// @param _bundle ABI-encoded InteropBundle to verify.
  /// @param _proof Inclusion proof for the bundle message.
  function verifyBundle(bytes memory _bundle, InteropMessageProof memory _proof)
    external;
  ...
  function callStatus(bytes32 bundleHash, uint256 callIndex)
    external
    view
    returns (CallStatus);
}
