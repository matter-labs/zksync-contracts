...
library InteroperableAddress {
  ...
  function formatV1(
    ...
    require(
      chainReference.length > 0 || addr.length > 0,
   ...
   */
  function formatEvmV1(uint256 chainid, address addr)
    internal
    pure
  ...
  function formatEvmV1(uint256 chainid) internal pure returns (bytes memory) {
    bytes memory chainReference = _toChainReference(chainid);
    return abi.encodePacked(
      bytes4(0x00010000), uint8(chainReference.length), chainReference, uint8(0)
    );
  }
    ...
    )
  {
    unchecked {
      ...
      addr = self.slice(
        0x06 + chainReferenceLength, 0x06 + chainReferenceLength + addrLength
      );
    }
  }
   ...
   */
  function parseEvmV1(bytes memory self)
    internal
    pure
    returns (uint256 chainId, address addr)
  {
    bool success;
   ...
   */
  function parseEvmV1Calldata(bytes calldata self)
    internal
    pure
    returns (uint256 chainId, address addr)
  {
   ...
   * return value to indicate parsing failure when the input does not follow version 1 of ERC-7930.
   */
  function tryParseEvmV1(bytes memory self)
    internal
    pure
  ...
  function _toChainReference(uint256 chainid)
    private
    pure
    returns (bytes memory)
  {
  ...
  function _readBytes2(bytes memory buffer, uint256 offset)
    private
    pure
  ...
  function _readBytes2Calldata(bytes calldata buffer, uint256 offset)
    private
    pure
    returns (bytes2 value)
  {
