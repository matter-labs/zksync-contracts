...
library TransactionValidator {
  ...
  function validateL1ToL2Transaction(
    ...
    uint256 l2GasForTxBody =
      getTransactionBodyGasLimit(_transaction.gasLimit, _encoded.length);
    ...
    if (
      l2GasForTxBody / _transaction.gasPerPubdataByteLimit
        > _priorityTxMaxPubdata
    ) {
      revert PubdataGreaterThanLimit(
        _priorityTxMaxPubdata,
        l2GasForTxBody / _transaction.gasPerPubdataByteLimit
      );
    }
    ...
    returns (uint256 batchOverheadForTransaction)
  {
    ...
    batchOverheadForTransaction =
      Math.max(batchOverheadForTransaction, overheadForLength);
  }
}
