...
library Utils {
  ...
  {
    unchecked {
      uint256 decodedCodeLength = uint256(uint8(_bytecodeHash[2])) * 256
  ...
  function hashEVMBytecode(
    ...
    hashedEVMBytecode = EfficientCall.sha(_paddedBytecode)
      & 0x00000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF;
  ...
  function getNewAddressCreate2EVM(
    ...
    bytes32 hash = keccak256(
      abi.encodePacked(
        bytes1(CREATE2_EVM_PREFIX), _sender, _salt, _bytecodeHash
      )
    );
    ...
    returns (address newAddress)
  {
    ...
    bytes memory listLengthEncoded =
      RLPEncoder.encodeListLen(uint64(listLength));

    bytes memory digest =
      bytes.concat(listLengthEncoded, addressEncoded, nonceEncoded);
